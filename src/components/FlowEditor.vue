...
const subCanvasContextKey = computed(() =>
  `${subCanvasSource.value}::${subCanvasFilename.value}::${subCanvas.value.nodeId}`
)
...
<template>
  ...
  >
    ...
    />
    <SubCanvasPanel
      :key="subCanvas.external ? `resource:${subCanvasContextKey}` : 'current-document'"
