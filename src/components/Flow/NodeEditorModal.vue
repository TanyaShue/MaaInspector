...
<template>
  ...
  >
    <div class="bg-white w-[720px] max-w-[95vw] rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[85vh]">

      <!-- 头部 -->
      <div class="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <div v-show="activeTab === 'properties'" class="p-5 space-y-4">
          ...
          <!-- 识别算法特有属性 -->
          <div v-if="currentRecognition !== 'DirectHit'" class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            <button 
            ...
            >
              ...
              <component :is="expandedSections.recognition ? ChevronDown : ChevronRight" :size="16" class="text-slate-400" />
            </button>
          ...
          <!-- 动作特有属性 -->
          <div v-if="!['DoNothing', 'StopTask'].includes(currentAction)" class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            <button 
        ...
        <div v-show="activeTab === 'json'" class="p-5 flex flex-col h-full">
          ...
          <!-- 错误提示 -->
          <div v-if="jsonError" class="flex items-center gap-2 text-red-600 text-sm bg-red-50 p-3 rounded-lg border border-red-200 mb-3">
            <AlertCircle :size="16" />
            <span>{{ jsonError }}</span>
          </div>
