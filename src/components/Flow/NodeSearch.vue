...
const localResults = computed(() => {
  ...
  if (useRegex.value) {
    try { regex = new RegExp(query, 'i') } catch (e) { return [] }
  }
...
<template>
  ...
  >
    <div
      ...
      @mousedown.stop
    >
      <div class="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-emerald-50 to-teal-50 border-b border-slate-100 cursor-move shrink-0" @mousedown="startDrag">
        ...
        <button @click.stop="$emit('close')" class="p-1.5 hover:bg-white/80 rounded-lg text-slate-400 hover:text-slate-600 transition-colors">
          <X :size="16" />
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/30">
        ...
        <div v-if="localResults.length > 0 && (otherFileResults.length > 0 || isSearchingRemote)" class="h-px bg-slate-200 mx-4 my-1"></div>
        ...
        <div v-if="searchQuery.trim()" class="py-2">
          ...
          <div v-if="otherFileResults.length === 0 && !isSearchingRemote && searchQuery" class="px-4 py-2 text-xs text-slate-400 italic">
            无其他文件匹配
          </div>
          ...
          >
            <div class="min-w-0 flex-1">
               <div class="flex items-center gap-2 mb-0.5">
                 ...
                 <span class="text-[10px] text-slate-500 truncate max-w-[150px]" :title="res.filename">{{ res.filename }}</span>
               </div>
