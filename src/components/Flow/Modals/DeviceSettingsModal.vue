    ...
    <div class="bg-white rounded-xl shadow-2xl border border-slate-200 flex overflow-hidden w-[650px] h-[500px]">
      <div class="w-[200px] bg-slate-50 border-r border-slate-100 flex flex-col">
        ...
        <div class="p-2 border-t border-slate-100 flex flex-col gap-2">
          <button @click="editingDevices.push({name:'New Device', address:'', config:{}})" class="border border-dashed border-slate-300 rounded-lg py-1.5 text-xs text-slate-500 hover:text-indigo-600 hover:border-indigo-300 hover:bg-white transition-colors flex items-center justify-center gap-1"><Plus :size="12" /> 手动添加</button>
          <button @click="handleSearch" :disabled="isSearching" class="bg-indigo-100 text-indigo-600 rounded-lg py-1.5 text-xs font-bold hover:bg-indigo-200 transition-colors flex items-center justify-center gap-1"><component :is="isSearching ? Loader2 : Radar" :size="12" :class="{'animate-spin': isSearching}" /> {{ isSearching ? '扫描中...' : '自动搜索' }}</button>
        </div>
      </div>
      <div class="flex-1 flex flex-col bg-white">
        ...
        <div v-if="editingDevices[editDevIndex]" class="flex-1 p-5 overflow-y-auto space-y-4">
           <div class="space-y-1"><label class="text-[10px] font-bold text-slate-400 uppercase">Name</label><input v-model="editingDevices[editDevIndex].name" class="w-full bg-white border border-slate-200 rounded-lg py-2 pr-3 text-xs text-slate-600 outline-none transition-all shadow-sm focus:border-indigo-300 focus:ring-2 focus:ring-indigo-50" /></div>
           <div class="space-y-1"><label class="text-[10px] font-bold text-slate-400 uppercase">Address</label><input v-model="editingDevices[editDevIndex].address" class="w-full bg-white border border-slate-200 rounded-lg py-2 pr-3 text-xs text-slate-600 outline-none transition-all shadow-sm focus:border-indigo-300 focus:ring-2 focus:ring-indigo-50 font-mono" /></div>
           <div class="space-y-1 flex-1 flex flex-col"><label class="text-[10px] font-bold text-slate-400 uppercase">Config</label><textarea :value="JSON.stringify(editingDevices[editDevIndex].config, null, 2)" @input="e => { try{editingDevices[editDevIndex].config = JSON.parse(e.target.value)}catch(err){} }" class="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg text-[10px] font-mono focus:outline-none focus:ring-1 focus:ring-indigo-200 resize-none flex-1"></textarea></div>
        ...
        <div class="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-2">
           ...
           <button @click="save" class="px-3 py-1.5 text-xs font-bold bg-indigo-500 text-white rounded shadow-sm hover:bg-indigo-600 transition-colors flex items-center gap-1"><Save :size="14" /> 保存</button>
        </div>
      </div>
    </div>
  </div>
