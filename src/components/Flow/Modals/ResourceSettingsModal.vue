      ...
      <div class="flex-1 flex flex-col bg-white">
        ...
        <div v-if="editingProfiles[editProfIndex]" class="flex-1 p-5 overflow-hidden flex flex-col gap-4">
          <div class="space-y-1">
            ...
            <input v-model="editingProfiles[editProfIndex].name" type="text"
            ...
            <div
              ...
              <div v-for="(path, pIdx) in editingProfiles[editProfIndex].paths" :key="pIdx"
                ...
                <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button @click="movePath(pIdx, -1)" :disabled="pIdx===0"
                          class="p-1 hover:bg-slate-100 rounded text-slate-400 hover:text-blue-500 disabled:opacity-30">
                    <ArrowUp :size="12"/>
                  </button>
                  <button @click="movePath(pIdx, 1)" :disabled="pIdx===editingProfiles[editProfIndex].paths.length-1"
                          class="p-1 hover:bg-slate-100 rounded text-slate-400 hover:text-blue-500 disabled:opacity-30">
                    <ArrowDown :size="12"/>
                  </button>
                  <button @click="removePath(pIdx)"
                          class="p-1 hover:bg-red-50 rounded text-slate-400 hover:text-red-500">
                    <Trash2 :size="12"/>
                  </button>
                </div>
              </div>
              <div v-if="editingProfiles[editProfIndex].paths.length === 0"
                   class="text-center py-4 text-xs text-slate-400 italic">暂无路径
              </div>
            </div>
          </div>
